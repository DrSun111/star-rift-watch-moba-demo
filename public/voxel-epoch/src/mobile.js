export class MobileControls{
  constructor({player,gameplay,ui,canvas,state}){
    ...
    this.enabled=(navigator.maxTouchPoints||0)>0||matchMedia('(pointer:coarse)').matches||/Android|Mobile|HarmonyOS/i.test(navigator.userAgent);
    this.movePointer=null;this.lookPointer=null;this.moveOrigin={x:0,y:0};this.lookLast={x:0,y:0};
    globalThis.__voxelBooted?.();
    if(!this.enabled)return;
    // Android/HarmonyOS WebView may not implement Pointer Lock. Touch look replaces it entirely.
    this.player.lock=()=>{};this.player.unlock=()=>{};
    document.documentElement.classList.add('mobile-game-root');
    ...
    document.body.classList.add('mobile-game');this.build();this.bind();this.fitViewport();
    addEventListener('resize',()=>this.fitViewport(),{passive:true});
    addEventListener('orientationchange',()=>setTimeout(()=>this.fitViewport(),120),{passive:true});
  }
  fitViewport(){
    const h=window.visualViewport?.height||innerHeight,w=window.visualViewport?.width||innerWidth;
    document.documentElement.style.setProperty('--app-h',`${h}px`);document.documentElement.style.setProperty('--app-w',`${w}px`);
  }
  prepareGesture(){globalThis.__voxelAudio?.unlock();try{screen.orientation?.lock?.('landscape')?.catch?.(()=>{});}catch{}}
  build(){
    const root=document.createElement('div');root.id='mobile-controls';root.innerHTML=`
      <div class="mobile-look-zone" data-look-zone aria-label="滑动控制视角"></div>
      <div class="mobile-stick" data-stick aria-label="移动摇杆"><div class="stick-ring"></div><div class="stick-knob" data-stick-knob></div></div>
      ...
      <div class="mobile-top-actions"><button data-act="pets">精灵</button><button data-act="bag">背包</button><button data-act="view">视角</button></div>
  ...
  bind(){
    ...
    this.stick.addEventListener('pointerdown',e=>{stop(e);this.prepareGesture();this.movePointer=e.pointerId;this.stick.setPointerCapture(e.pointerId);const r=this.stick.getBoundingClientRect();this.moveOrigin={x:r.left+r.width/2,y:r.top+r.height/2};this.updateStick(e.clientX,e.clientY);});
    ...
    this.lookZone.addEventListener('pointerdown',e=>{stop(e);this.prepareGesture();this.lookPointer=e.pointerId;this.lookZone.setPointerCapture(e.pointerId);this.lookLast={x:e.clientX,y:e.clientY};});
    this.lookZone.addEventListener('pointermove',e=>{if(e.pointerId!==this.lookPointer)return;stop(e);const dx=e.clientX-this.lookLast.x,dy=e.clientY-this.lookLast.y;this.lookLast={x:e.clientX,y:e.clientY};const scale=Math.max(.72,Math.min(1.2,(window.devicePixelRatio||1)/2));this.player.yaw-=dx*.00315*scale;this.player.pitch=Math.max(-1,Math.min(.62,this.player.pitch-dy*.00225*scale));this.state.lookYaw=this.player.yaw;this.state.lookPitch=this.player.pitch;});
    ...
    this.root.querySelectorAll('[data-act]').forEach(btn=>{const act=btn.dataset.act;if(['jump','down','sprint'].includes(act)){btn.addEventListener('pointerdown',e=>{stop(e);this.prepareGesture();this.hold(act,true);});for(const ev of ['pointerup','pointercancel','pointerleave'])btn.addEventListener(ev,e=>{stop(e);this.hold(act,false);});}else btn.addEventListener('pointerdown',e=>{stop(e);this.prepareGesture();this.press(act);});});
    addEventListener('blur',()=>this.releaseAll());document.addEventListener('visibilitychange',()=>{if(document.hidden)this.releaseAll();});
  }
  releaseAll(){this.movePointer=null;this.lookPointer=null;this.setKeys(0,0);this.player.keys.ShiftLeft=false;this.player.keys.Space=false;this.player.keys.ControlLeft=false;if(this.knob)this.knob.style.transform='translate(0px,0px)';}
  setKeys(strafe,forward){const k=this.player.keys;k.KeyW=forward>.18;k.KeyS=forward<-.18;k.KeyD=strafe>.18;k.KeyA=strafe<-.18;}
  updateStick(x,y){const dx=x-this.moveOrigin.x,dy=y-this.moveOrigin.y,max=Math.max(34,this.stick.clientWidth*.42),len=Math.hypot(dx,dy)||1,scale=Math.min(1,max/len),cx=dx*scale,cy=dy*scale;this.knob.style.transform=`translate(${cx}px,${cy}px)`;this.setKeys(cx/max,-cy/max);}
